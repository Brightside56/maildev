...
function saveEmailToStore (id, isRead = false, envelope, parsedEmail) {
  ...
  if (mailServer.mailLifeSpan > 0) {
    scheduler.resetTime(serialized.time.getTime() + mailServer.mailLifeSpan)
  }
}
...
mailServer.create = function (port, host, mailDir, user, password, hideExtensions, mailLifeSpan) {
...
mailServer.onSmtpError = function (err) {
  if (err.code === 'ECONNRESET' && err.syscall === 'read') {
    ...
    logger.warn(`Ignoring "${err.message}" error thrown by SMTP server. Full error details below.`)
    logger.error(err)
  }
}
